...
test('save/load preserves permanent modifications, all combat zones, copies, powers and independent workflow cursors',()=>{
  const game=new Game({seed:'save'});game.run.deck[0].upgrade=1;game.run.deck[0].modifiers.push({costDelta:-1,copyable:true});game.enterNode('0-0');const e=game.combat;
...
test('saving a reward or ending does not accidentally resume the finished combat',()=>{
  const game=new Game();game.enterNode('0-0');game.combat.state.enemies.forEach(e=>e.hp=0);game.combat.checkOutcome();game.finishCombat();const copy=roundtrip(game);assert.equal(copy.run.screen,'reward');
...
test('reloading finished combat does not fire combat start or combat end twice',()=>{const game=new Game();game.enterNode('0-0');game.debug('win');const count=game.combat.state.log.length;const copy=roundtrip(game);assert.equal(copy.combat.state.log.length,count);assert.equal(copy.combat.state.outcome,'victory');});
test('node can only be entered from map through a current connection',()=>{const game=new Game();assert.throws(()=>game.enterNode('3-0'));game.enterNode('0-0');assert.throws(()=>game.enterNode('1-0'));});
...
test('events charge once, enforce affordability and apply deck choices',()=>{const game=new Game();game.run.map.nodes[0].type='Event';game.enterNode('0-0');game.run.nodeState.eventId='vending';game.run.gold=0;assert.throws(()=>game.eventChoice(0));assert.equal(game.run.nodeState.resolved,undefined);game.eventChoice(1);assert.equal(game.run.hp,game.run.maxHp-5);assert.equal(game.run.nodeState.selection,'upgrade');assert.throws(()=>game.eventChoice(1));});
test('single-act vertical slice reaches victory via connected nodes and preserves run summary',()=>{
  ...
  while(game.run.screen!=='victory') {
    ...
    if(game.run.screen==='combat'){game.combat.state.enemies.forEach(e=>e.hp=0);game.combat.checkOutcome();game.finishCombat();game.claimReward();}
    else {if(game.run.nodeState.type==='Treasure')game.treasure();if(game.run.nodeState.type==='Rest')game.rest();game.leaveNode();}
  }
...
test('real card actions and enemy turns can defeat Boss without debug tools',()=>{
  const game=new Game({seed:'boss-proof'});game.run.deck=['strike','strike','defend','defend','routine','routine','coffee','staple','power','weaken'].map((id,i)=>createCard(id,`proof-${i}`,{upgrade:1}));game.run.hp=game.run.maxHp;game.startCombat(encounters.director);let turns=0;
