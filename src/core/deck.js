export class DeckManager {
  ...
  draw(amount) {
    for (let i = 0; i < amount; i++) {
      ...
      this.state.hand.push(card); this.engine.bus.emit('card:drawn', { cardId: card.id });
    }
  }
  ...
  endTurn() {
    for (const card of [...this.state.hand]) {
      ...
      else if (!keywords.includes('Retain')) { this.remove(card.id); this.state.discard.push(card); }
    }
  }
  move(from, to, amount) {
